...
export const render = () => {
    ...
    const GetExpenses = async () => {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            div1.innerHTML =
                'Budget Name : ' + data[i].budgetName + '<br/><br/>';

            newExpensesContainerEl.appendChild(div1);

            for (let k = 0; k < data[i].categories.length; i++) {
                debugger
                let div3 = document.createElement("div");
                div3.setAttribute('class', 'category-name')
                div3.innerHTML = 'Category Name : ' + data[i].categories[k].categoryName + '<br/><br/>' + 'Expenses :' + '<br/>';
                ...
                for (let j = 0; j < data[i].categories[k].expenses.length; j++) {
                    ...
                    div2.innerHTML =
                        'Amount : ' + data[i].categories[k].expenses[j].amount + '<br/>' +
                        'Recipient : ' + data[i].categories[k].expenses[j].recipient + '<br/>' +
                        'Date : ' + data[i].categories[k].expenses[j].date + '<br/>' +
                        'Comment : ' + data[i].categories[k].expenses[j].comment + '<br/><br/>';
